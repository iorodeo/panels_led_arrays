...
if __name__ == '__main__':
    ...
    if xNum%2==0:
        xStart = xOffset + 0.5*xPcb - xLed*(int(xNum/2) - 0.5)
    else:
        xStart = xOffset + 0.5*xPcb - xLed*int(xNum/2)

    if yNum%2==0:
        yStart = yOffset + 0.5*yPcb - yLed*(int(yNum/2) - 0.5)
    else:
        yStart = yOffset + 0.5*yPcb - yLed*int(yNum/2)
    ...
    xHeader  = xOffset + 0.5*xPcb 
    yHeader1 = yOffset + 0.5*yPcb + 1*yLed  
    yHeader2 = yOffset + 0.5*yPcb - 1*yLed
    angHeader = 0
    ...
    for i in range(0,xNum):
        for j in range(0,yNum):
            ...
            placer.setModulePos(name,x,y,angLed)
