...
if __name__ == '__main__':
    ...
    yHeaderList = []
    yHeaderList.append(yOffset + 0.5*yPcb + 1*yLed)  
    yHeaderList.append(yOffset + yPcb- 4.0/25.4)
    yHeaderList.append(yOffset + 0.5*yPcb - 1*yLed)
    yHeaderList.append(yOffset + 4.0/25.4)

    placer = ComponentPlacer('led_array_adaptor.brd')
    for i, yHeader in enumerate(yHeaderList):
        placer.setModulePos('P{0}'.format(i+1),xHeader, yHeader, angHeader)
